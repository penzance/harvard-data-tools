...
public class EnrollmentappPhase0 extends Phase0 {
  ...
  public EnrollmentappPhase0(final EnrollmentappDataConfig config, final String runId,
  ...
  protected ReturnStatus run() throws IOException, InterruptedException, ExecutionException {
    log.info("Starting Enrollmentapp Phase0...");
    // Start
    ...
    for (final S3ObjectSummary obj : aws.listKeys(config.getDropboxBucket())) {
        if (obj.getKey().endsWith(".gz")) {
            ...
            final EnrollmentappSingleFileParser parser = new EnrollmentappSingleFileParser(obj,
...
class EnrollmentappSingleFileParser implements Callable<InputTableIndex> {
	  private static final Logger log = LogManager.getLogger();
	  ...
	  public EnrollmentappSingleFileParser(final S3ObjectSummary inputObj, final S3ObjectId outputLocation,
	      final EnrollmentappDataConfig config) {
