...
public class EdxCodeGenerator extends CodeGenerator {

  private static final int TRANFORMATION_PHASES = 2;

  private static final String PHASE_ZERO_PACKAGE = "edu.harvard.data.edx.bindings.phase0";
  private static final String PHASE_ONE_PACKAGE = "edu.harvard.data.edx.bindings.phase1";
  private static final String PHASE_TWO_PACKAGE = "edu.harvard.data.edx.bindings.phase2";
  private static final String PHASE_THREE_PACKAGE = "edu.harvard.data.edx.bindings.phase3";
  private static final String IDENTITY_HADOOP_PACKAGE = "edu.harvard.data.edx.identity";

  public static final String PHASE_ZERO_TABLES_JSON = "edx/phase0_redshift_tables.json";
  public static final String PHASE_ONE_IDENTIFIERS_JSON = "edx/phase1_identifiers.json";
  public static final String PHASE_TWO_ADDITIONS_JSON = "edx/phase2_schema_additions.json";
  public static final String PHASE_THREE_ADDITIONS_JSON = "edx/phase3_schema_additions.json";
  ...
  public EdxCodeGenerator(final String schemaVersion, final File gitDir, final File codeDir,
      final EdxDataConfig config, final String runId) throws FileNotFoundException {
  ...
  UnexpectedApiResponseException, SQLException, VerificationException {
    ...
    final EdxDataConfig config = EdxDataConfig
        .parseInputFiles(EdxDataConfig.class, configFiles, false);
    new EdxCodeGenerator(schemaVersion, gitDir, dir, config, runId).generate();
  }
  ...
  protected GenerationSpec createGenerationSpec() throws IOException, VerificationException {
    ...
    spec.setJavaProjectName("edx_generated_code");
    spec.setJavaTableEnumName("EdxTable");
    ...
    spec.setHadoopIdentityManagerClass("EdxIdentityHadoopManager");
    spec.setMainIdentifier(IdentifierType.HUID);
    spec.setHiveScriptDir(new File(gitDir, "hive/edx"));
  ...
  private static DataSchema readSchema(final String version)
    ...
    final ClassLoader classLoader = EdxCodeGenerator.class.getClassLoader();
    try (final InputStream in = classLoader
        .getResourceAsStream("edx_schema_" + version + ".json")) {
