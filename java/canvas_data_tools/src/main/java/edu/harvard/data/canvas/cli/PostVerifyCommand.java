...
public class PostVerifyCommand implements Command {
  ...
  @Option(name = "-data", usage = "Location of processed data files. Required for phases > 0.", metaVar = "/path/to/data", required = false)
  ...
  private Verifier getVerifier(final DataConfiguration config, final ExecutorService exec)
    ...
    case 1:
      return new Phase1PostVerifier(config, hadoopConfig, hdfsService, dataDir, verifyDir);
    case 2:
  ...
  private boolean checkArguments() {
    if (phase == 0) {
      ...
      if (verifyDir == null) {
        log.error("Verify directory is required for all phases other than zero.");
        return false;
      }
      if (!verifyDir.toLowerCase().startsWith("hdfs://")) {
        verifyDir = "hdfs://" + verifyDir;
      }
    }
    return true;
  }
