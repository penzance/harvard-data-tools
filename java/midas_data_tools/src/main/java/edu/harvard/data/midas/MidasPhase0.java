...
public class MidasPhase0 extends Phase0 {
  ...
  public MidasPhase0(final MidasDataConfig config, final String runId,
  ...
  protected ReturnStatus run() throws IOException, InterruptedException, ExecutionException {
    log.info("Starting Midas Phase0...");
    // Start
    ...
    for (final S3ObjectSummary obj : aws.listKeys(config.getDropboxBucket())) {
        if (obj.getKey().endsWith(".gz")) {
            ...
            final MidasSingleFileParser parser = new MidasSingleFileParser(obj,
...
class MidasSingleFileParser implements Callable<InputTableIndex> {
	  private static final Logger log = LogManager.getLogger();
	  ...
	  public MidasSingleFileParser(final S3ObjectSummary inputObj, final S3ObjectId outputLocation,
	      final MidasDataConfig config) {
