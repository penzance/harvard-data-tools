...
public class EventJsonDocumentParser implements JsonDocumentParser {
  ...
  public Map<String, List<? extends DataTable>> getDocuments(final Map<String, Object> values)
    ...
    // Primary Roles 
    final Phase0PrimaryRoles primaryrole = new Phase0PrimaryRoles(format, values);
    final List<Phase0PrimaryRoles> primaryroles = new ArrayList<Phase0PrimaryRoles>();
    primaryroles.add(primaryrole);
    tables.put("PrimaryRoles", primaryroles );
  ...
  public void verifyParser(final Map<String, Object> values,
    ...
    // Start
	final List<? extends DataTable> primaryroles = tables.get("PrimaryRoles");


	final Map<String, Object> parsedPrimaryRoles = primaryroles.get(0).getFieldsAsMap();
	
	// Primary Roles
	if (dataproduct.equals("PrimaryRoles")) {
		
	    try {
		    compareMaps(values, parsedPrimaryRoles );
	    } catch (final VerificationException e) {
	        log.error("Failed to verify JSON document. " + e.getMessage());
	        log.error("Original map: " + values);
	        log.error("Parsed map:   " + parsedPrimaryRoles );
	        throw e;			
	    }
	}
  }
