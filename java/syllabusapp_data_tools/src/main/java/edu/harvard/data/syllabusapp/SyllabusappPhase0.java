...
public class SyllabusappPhase0 extends Phase0 {
  ...
  public SyllabusappPhase0(final SyllabusappDataConfig config, final String runId,
  ...
  protected ReturnStatus run() throws IOException, InterruptedException, ExecutionException {
    log.info("Starting Syllabusapp Phase0...");
    // Start
    ...
    for (final S3ObjectSummary obj : aws.listKeys(config.getDropboxBucket())) {
        if (obj.getKey().endsWith(".gz")) {
            ...
            final SyllabusappSingleFileParser parser = new SyllabusappSingleFileParser(obj,
...
class SyllabusappSingleFileParser implements Callable<InputTableIndex> {
	  private static final Logger log = LogManager.getLogger();
	  ...
	  public SyllabusappSingleFileParser(final S3ObjectSummary inputObj, final S3ObjectId outputLocation,
	      final SyllabusappDataConfig config) {
