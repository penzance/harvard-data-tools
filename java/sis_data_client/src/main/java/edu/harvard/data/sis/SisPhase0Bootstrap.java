...
import edu.harvard.data.sis.BootstrapParameters;
import edu.harvard.data.sis.SisPhase0Bootstrap;
import edu.harvard.data.pipeline.Phase0Bootstrap;
import edu.harvard.data.schema.UnexpectedApiResponseException;

public class SisPhase0Bootstrap extends Phase0Bootstrap
implements RequestStreamHandler, RequestHandler<BootstrapParameters, String> {
  ...
  public static void main(final String[] args) 
	...
	System.out.println(new SisPhase0Bootstrap().handleRequest(params, null));
  }	
  ...
  public String handleRequest(final BootstrapParameters params, final Context context) {
	try {
	      ...
	      super.init(params.getConfigPathString(), SisDataConfig.class, params.getCreatePipeline());
	      super.run(context);
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, final Context context) {
	try {
	  ...
	  super.init(params.getConfigPathString(), 
	    		 SisDataConfig.class, params.getCreatePipeline(), requestjson);
	  super.run(context);
	} catch (IOException | DataConfigurationException | UnexpectedApiResponseException e) {
  ...
  protected boolean newDataAvailable()
	...
	return !aws.listKeys(((SisDataConfig)config).getDropboxBucket()).isEmpty();
  }
