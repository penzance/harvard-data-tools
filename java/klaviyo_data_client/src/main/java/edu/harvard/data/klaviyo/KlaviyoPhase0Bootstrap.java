...
import edu.harvard.data.klaviyo.BootstrapParameters;
import edu.harvard.data.klaviyo.KlaviyoPhase0Bootstrap;
...
implements RequestStreamHandler, RequestHandler<BootstrapParameters, String> {
  ...
  public static void main(final String[] args) 
	...
	System.out.println(new KlaviyoPhase0Bootstrap().handleRequest(params, null));
  }	
  ...
  public String handleRequest(final BootstrapParameters params, final Context context) {
	try {
	      ...
	      super.init(params.getConfigPathString(), KlaviyoDataConfig.class, params.getCreatePipeline());
	      super.run(context);
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, final Context context) {
	try {
	  ...
	  super.init(params.getConfigPathString(), 
	    		 KlaviyoDataConfig.class, params.getCreatePipeline(), requestjson);
	  super.run(context);
	} catch (IOException | DataConfigurationException | UnexpectedApiResponseException e) {
  ...
  protected boolean newDataAvailable()
	...
	return !aws.listKeys(((KlaviyoDataConfig)config).getDropboxBucket()).isEmpty();
  }
