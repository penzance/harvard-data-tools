...
import edu.harvard.data.syllabusapp.BootstrapParameters;
import edu.harvard.data.syllabusapp.SyllabusappPhase0Bootstrap;
import edu.harvard.data.pipeline.Phase0Bootstrap;
import edu.harvard.data.schema.UnexpectedApiResponseException;

public class SyllabusappPhase0Bootstrap extends Phase0Bootstrap
  ...
  public static void main(final String[] args) 
	...
	System.out.println(new SyllabusappPhase0Bootstrap().handleRequest(params, null));
  }	
  ...
  public String handleRequest(final BootstrapParameters params, final Context context) {
    try {
      ...
      super.init(params.getConfigPathString(), SyllabusappDataConfig.class, params.getCreatePipeline() );
      super.run(context);
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, final Context context) {
	try {
	  ...
	  super.init(params.getConfigPathString(), 
	    		 SyllabusappDataConfig.class, params.getCreatePipeline(), requestjson);
	  super.run(context);
	} catch (IOException | DataConfigurationException | UnexpectedApiResponseException e) {
  ...
  protected boolean newDataAvailable()
    ...
    return !aws.listKeys(((SyllabusappDataConfig)config).getDropboxBucket()).isEmpty();
  }
