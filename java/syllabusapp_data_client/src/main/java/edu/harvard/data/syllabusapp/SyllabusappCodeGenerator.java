...
public class SyllabusappCodeGenerator extends CodeGenerator {

  private static final Logger log = LogManager.getLogger();

  private static final int TRANFORMATION_PHASES = 2;

  private static final String PHASE_ZERO_PACKAGE = "edu.harvard.data.syllabusapp.bindings.phase0";
  private static final String PHASE_ONE_PACKAGE = "edu.harvard.data.syllabusapp.bindings.phase1";
  private static final String PHASE_TWO_PACKAGE = "edu.harvard.data.syllabusapp.bindings.phase2";
  private static final String PHASE_THREE_PACKAGE = "edu.harvard.data.syllabusapp.bindings.phase3";
  private static final String IDENTITY_HADOOP_PACKAGE = "edu.harvard.data.syllabusapp.identity";

  public static final String PHASE_ZERO_TABLES_JSON = "syllabusapp/phase0_redshift_tables.json";
  public static final String PHASE_ONE_IDENTIFIERS_JSON = "syllabusapp/phase1_identifiers.json";
  public static final String PHASE_TWO_ADDITIONS_JSON = "syllabusapp/phase2_schema_additions.json";
  public static final String PHASE_THREE_ADDITIONS_JSON = "syllabusapp/phase3_schema_additions.json";
  ...
  public SyllabusappCodeGenerator(final String schemaVersion, final File gitDir, final File codeDir,
      final SyllabusappDataConfig config, final String runId, BootstrapParameters bootstrapParams ) throws FileNotFoundException {
  ...
  UnexpectedApiResponseException, SQLException, VerificationException {
    ...
    final SyllabusappDataConfig config = SyllabusappDataConfig
        .parseInputFiles(SyllabusappDataConfig.class, configFiles, false, bootstrapParamsString );
        log.info(args[5]);
    ...
    new SyllabusappCodeGenerator(schemaVersion, gitDir, dir, config, runId, bootstrapParams).generate();
  }
  ...
  protected GenerationSpec createGenerationSpec() throws IOException, VerificationException {
    ...
    spec.setJavaProjectName("syllabusapp_generated_code");
    spec.setJavaTableEnumName("SyllabusappTable");
    ...
    spec.setHadoopIdentityManagerClass("SyllabusappIdentityHadoopManager");
    spec.setMainIdentifier(IdentifierType.HUID);
    spec.setHiveScriptDir(new File(gitDir, "hive/syllabusapp"));
  ...
  private static DataSchema readSchema(final String version)
    ...
    final ClassLoader classLoader = SyllabusappCodeGenerator.class.getClassLoader();
    try (final InputStream in = classLoader
        .getResourceAsStream("syllabusapp_schema_" + version + ".json")) {
