...
public class EnrollmentappCodeGenerator extends CodeGenerator {

  private static final Logger log = LogManager.getLogger();

  private static final int TRANFORMATION_PHASES = 2;

  private static final String PHASE_ZERO_PACKAGE = "edu.harvard.data.enrollmentapp.bindings.phase0";
  private static final String PHASE_ONE_PACKAGE = "edu.harvard.data.enrollmentapp.bindings.phase1";
  private static final String PHASE_TWO_PACKAGE = "edu.harvard.data.enrollmentapp.bindings.phase2";
  private static final String PHASE_THREE_PACKAGE = "edu.harvard.data.enrollmentapp.bindings.phase3";
  private static final String IDENTITY_HADOOP_PACKAGE = "edu.harvard.data.enrollmentapp.identity";

  public static final String PHASE_ZERO_TABLES_JSON = "enrollmentapp/phase0_redshift_tables.json";
  public static final String PHASE_ONE_IDENTIFIERS_JSON = "enrollmentapp/phase1_identifiers.json";
  public static final String PHASE_TWO_ADDITIONS_JSON = "enrollmentapp/phase2_schema_additions.json";
  public static final String PHASE_THREE_ADDITIONS_JSON = "enrollmentapp/phase3_schema_additions.json";

  private final String schemaVersion;
  private final File gitDir;

  private final EnrollmentappDataConfig config;
  private final BootstrapParameters bootstrapParams;

  public EnrollmentappCodeGenerator(final String schemaVersion, final File gitDir, final File codeDir,
      final EnrollmentappDataConfig config, final String runId, BootstrapParameters bootstrapParams ) throws FileNotFoundException {
  ...
  UnexpectedApiResponseException, SQLException, VerificationException {
    ...
    final EnrollmentappDataConfig config = EnrollmentappDataConfig
        .parseInputFiles(EnrollmentappDataConfig.class, configFiles, false, bootstrapParamsString );
        log.info(args[5]);
    ...
    new EnrollmentappCodeGenerator(schemaVersion, gitDir, dir, config, runId, bootstrapParams).generate();
  }
  ...
  protected GenerationSpec createGenerationSpec() throws IOException, VerificationException {
    ...
    spec.setJavaProjectName("enrollmentapp_generated_code");
    spec.setJavaTableEnumName("EnrollmentappTable");
    ...
    spec.setHadoopIdentityManagerClass("EnrollmentappIdentityHadoopManager");
    spec.setMainIdentifier(IdentifierType.HUID);
    spec.setHiveScriptDir(new File(gitDir, "hive/enrollmentapp"));
  ...
  private static DataSchema readSchema(final String version)
    ...
    final ClassLoader classLoader = EnrollmentappCodeGenerator.class.getClassLoader();
    try (final InputStream in = classLoader
        .getResourceAsStream("enrollmentapp_schema_" + version + ".json")) {
