...
import edu.harvard.data.enrollmentapp.BootstrapParameters;
import edu.harvard.data.enrollmentapp.EnrollmentappPhase0Bootstrap;
import edu.harvard.data.pipeline.Phase0Bootstrap;
import edu.harvard.data.schema.UnexpectedApiResponseException;

public class EnrollmentappPhase0Bootstrap extends Phase0Bootstrap
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, final Context context) {
	try {
	  ...
	  super.init(params.getConfigPathString(), 
	    		 EnrollmentappDataConfig.class, params.getCreatePipeline(), requestjson);
	  super.run(context);
	} catch (IOException | DataConfigurationException | UnexpectedApiResponseException e) {
  ...
  protected boolean newDataAvailable()
    ...
    return !aws.listKeys(((EnrollmentappDataConfig)config).getDropboxBucket()).isEmpty();
  }
