...
public class MatterhornDownloadAndVerify extends DownloadAndVerify {
  ...
  public MatterhornDownloadAndVerify(final MatterhornDataConfig config, final String runId,
  ...
  @Override
  protected InputTableIndex run() throws IOException, InterruptedException, ExecutionException {
    log.info("Parsing files");
    final AwsUtils aws = new AwsUtils();
    final InputTableIndex dataIndex = new InputTableIndex();
    final List<Future<InputTableIndex>> jobs = new
        ArrayList<Future<InputTableIndex>>();
    for (final S3ObjectSummary obj : aws.listKeys(config.getDropboxBucket()))
    {
      if (obj.getKey().endsWith(".gz")) {
        final S3ObjectId outputLocation =
            AwsUtils.key(config.getS3WorkingLocation(runId));
        final MatterhornSingleFileParser parser = new
            MatterhornSingleFileParser(obj,
    ...
    //    return aws.readJson(
    //        AwsUtils.key("s3://hdt-working/Matterhorn/dce_matterhorn_simplified/directoryList.json"),
