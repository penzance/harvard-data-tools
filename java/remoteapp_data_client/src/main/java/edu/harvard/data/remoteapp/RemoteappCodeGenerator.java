...
public class RemoteappCodeGenerator extends CodeGenerator {

  private static final Logger log = LogManager.getLogger();

  private static final int TRANFORMATION_PHASES = 2;

  private static final String PHASE_ZERO_PACKAGE = "edu.harvard.data.remoteapp.bindings.phase0";
  private static final String PHASE_ONE_PACKAGE = "edu.harvard.data.remoteapp.bindings.phase1";
  private static final String PHASE_TWO_PACKAGE = "edu.harvard.data.remoteapp.bindings.phase2";
  private static final String PHASE_THREE_PACKAGE = "edu.harvard.data.remoteapp.bindings.phase3";
  private static final String IDENTITY_HADOOP_PACKAGE = "edu.harvard.data.remoteapp.identity";

  public static final String PHASE_ZERO_TABLES_JSON = "remoteapp/phase0_redshift_tables.json";
  public static final String PHASE_ONE_IDENTIFIERS_JSON = "remoteapp/phase1_identifiers.json";
  public static final String PHASE_TWO_ADDITIONS_JSON = "remoteapp/phase2_schema_additions.json";
  public static final String PHASE_THREE_ADDITIONS_JSON = "remoteapp/phase3_schema_additions.json";
  ...
  public RemoteappCodeGenerator(final String schemaVersion, final File gitDir, final File codeDir,
      final RemoteappDataConfig config, final String runId, BootstrapParameters bootstrapParams ) throws FileNotFoundException {
  ...
  UnexpectedApiResponseException, SQLException, VerificationException {
    ...
    final RemoteappDataConfig config = RemoteappDataConfig
        .parseInputFiles(RemoteappDataConfig.class, configFiles, false, bootstrapParamsString );
        log.info(args[5]);
    ...
    new RemoteappCodeGenerator(schemaVersion, gitDir, dir, config, runId, bootstrapParams).generate();
  }
  ...
  protected GenerationSpec createGenerationSpec() throws IOException, VerificationException {
    ...
    spec.setJavaProjectName("remoteapp_generated_code");
    spec.setJavaTableEnumName("RemoteappTable");
    ...
    spec.setHadoopIdentityManagerClass("RemoteappIdentityHadoopManager");
    spec.setMainIdentifier(IdentifierType.HUID);
    spec.setHiveScriptDir(new File(gitDir, "hive/remoteapp"));
  ...
  private static DataSchema readSchema(final String version)
    ...
    final ClassLoader classLoader = RemoteappCodeGenerator.class.getClassLoader();
    try (final InputStream in = classLoader
        .getResourceAsStream("remoteapp_schema_" + version + ".json")) {
