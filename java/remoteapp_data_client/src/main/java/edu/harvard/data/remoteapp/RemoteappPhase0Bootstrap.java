...
import edu.harvard.data.remoteapp.BootstrapParameters;
import edu.harvard.data.remoteapp.RemoteappPhase0Bootstrap;
import edu.harvard.data.schema.UnexpectedApiResponseException;

public class RemoteappPhase0Bootstrap extends Phase0Bootstrap
  ...
  public static void main(final String[] args) 
		...
		System.out.println(new RemoteappPhase0Bootstrap().handleRequest(params, null));
  }	

  @Override
  public String handleRequest(final BootstrapParameters params, final Context context) {
    try {
      super.init(params.getConfigPathString(), RemoteappDataConfig.class, params.getCreatePipeline() );
      super.run(context);
  ...
  public void handleRequest(InputStream inputStream, OutputStream outputStream, final Context context) {
	try {
	  ...
	  super.init(params.getConfigPathString(), 
	    		 RemoteappDataConfig.class, params.getCreatePipeline(), requestjson);
	  super.run(context);
	} catch (IOException | DataConfigurationException | UnexpectedApiResponseException e) {
  ...
  protected boolean newDataAvailable()
    ...
    return !aws.listKeys(((RemoteappDataConfig)config).getDropboxBucket()).isEmpty();
  }
