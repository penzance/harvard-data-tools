...
import edu.harvard.data.midas.BootstrapParameters;
import edu.harvard.data.midas.MidasPhase0Bootstrap;
...
implements RequestStreamHandler, RequestHandler<BootstrapParameters, String> {
  ...
  public static void main(final String[] args) 
	...
	System.out.println(new MidasPhase0Bootstrap().handleRequest(params, null));
  }	
		
  @Override
  public String handleRequest(final BootstrapParameters params, final Context context) {
	try {
	      super.init(params.getConfigPathString(), MidasDataConfig.class, true);
	      super.run(context);
  ...
  protected boolean newDataAvailable()
	...
	return !aws.listKeys(((MidasDataConfig)config).getDropboxBucket()).isEmpty();
  }
