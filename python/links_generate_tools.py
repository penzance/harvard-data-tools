...
# - LINKS_DATA_SCHEMA_VERSION: The version of the LINKS Data schema for which
...
DATA_TOOLS_DIR = "{0}/java/links_data_tools".format(GIT_BASE)
LINKS_DATA_CLIENT_DIR = "{0}/java/links_data_client".format(GIT_BASE)
...
def clean_up_files():
    os.rename(
        "{0}/target/links_data_tools-1.0.0.jar".format(DATA_TOOLS_DIR),
...
def run_generator():
    generator_classpath = "{0}:{1}:{2}".format(
        "{0}/target/links_data_client-1.0.0.jar".format(LINKS_DATA_CLIENT_DIR),
        "{0}/target/data_client-1.0.0.jar".format(DATA_CLIENT_DIR),
        SCHEMA_JSON_DIR,
    )
    main_class = "edu.harvard.data.links.LinksCodeGenerator"
    command = [
...
compile_java(LINKS_DATA_CLIENT_DIR)
run_generator()
